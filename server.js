...
app.post('/add-task' , async(req,res) =>{
    try{
        ...
        const read = JSON.parse(await fsPromise.readFile(path.join(__dirname , 'data' , 'tasks.json'), 'utf-8'))
        read.push(`${yazi}`)
        await fsPromise.writeFile(path.join(__dirname , 'data' , 'tasks.json'), JSON.stringify(read , null , 2))
        res.redirect('/')
    }catch(error){
...
app.get('/api/tasks' , async(req,res)=>{
   try{const data = JSON.parse(await fsPromise.readFile(path.join(__dirname , 'data' , 'tasks.json')))
   res.json(data)
   }
   catch(error){
    console.error(error);
